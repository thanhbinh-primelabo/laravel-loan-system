                ...
                <div class="content-group">
                            ...
                            @if (!empty($borrower->photo))
                                <a href="{{ asset('uploads/' . $borrower->photo) }}"
                                    class="display-inline-block content-group-sm fancybox"> <img
                                        class="img-circle img-responsive" src="{{ asset('uploads/' . $borrower->photo) }}"
                                        alt="user image" style="max-height: 120px!important;" /></a>
                            @else
                                <a href="#" class="display-inline-block content-group-sm ">
                                    < <img class="img-circle img-responsive" src="{{ asset('assets/dist/img/user.png') }}"
                    ...
                    <div class="panel no-border-top no-border-radius-top">
                        <ul class="navigation">
                            ...
                            <li><a href="#loans" data-toggle="tab"><i class="icon-balance"></i>
                                    {{ trans_choice('general.loan', 2) }}</a></li>
                            <li><a href="#payments" data-toggle="tab"><i class="icon-coin-dollar"></i>
                                    {{ trans_choice('general.payment', 2) }} </a>
                            </li>
                ...
                <div class="tab-pane fade in active" id="profile">
                    <div class="panel panel-white">
                                    ...
                                    <ul class="dropdown-menu" role="menu">
                                        <li>
                                            <a href="{{ url('loan/' . $borrower->id . '/borrower_statement/print') }}"
                                                target="_blank"><i class="icon-printer"></i>
                                                {{ trans_choice('general.print', 1) }}
                                                {{ trans_choice('general.statement', 1) }}
                                            </a>
                                        </li>
                                        <li>
                                            <a href="{{ url('loan/' . $borrower->id . '/borrower_statement/pdf') }}"
                                                ...
                                                {{ trans_choice('general.in', 1) }} {{ trans_choice('general.pdf', 1) }}
                                            </a>
                                        </li>
                                        <li>
                        ...
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-md-6">
                                    <h6><b>{{ trans_choice('general.basic', 1) }}
                                            {{ trans_choice('general.detail', 2) }}</b>
                                    </h6>
                                ...
                                <div class="col-md-6">
                                    <h6>
                                        <b>{{ trans_choice('general.contact', 1) }}
                                            {{ trans_choice('general.detail', 2) }}</b>
                                    </h6>
                                    <table class="table table-striped table-hover">
                                        <tr>
                                    ...
                                    <tbody>
                                        @foreach ($borrower->loans as $key)
                                            <tr>
                                                ...
                                                <td>{{ round(\App\Helpers\GeneralHelper::loan_total_due_amount($key->id), 2) }}
                                                </td>
                                                <td>{{ round(\App\Helpers\GeneralHelper::loan_total_paid($key->id), 2) }}
                                                </td>
                                                <td>{{ round(\App\Helpers\GeneralHelper::loan_total_balance($key->id), 2) }}
                                                </td>
                                                <td>
                                                    @if ($key->maturity_date < date('Y-m-d') && \App\Helpers\GeneralHelper::loan_total_balance($key->id) > 0)
                                                        <span
                                                            class="label label-danger">{{ trans_choice('general.past_maturity', 1) }}</span>
                                                    @else
                                                        @if ($key->status == 'pending')
                                                            <span
                                                                class="label label-warning">{{ trans_choice('general.pending', 1) }}
                                                                {{ trans_choice('general.approval', 1) }}</span>
                                                        @endif
                                                        @if ($key->status == 'approved')
                                                            <span
                                                                class="label label-info">{{ trans_choice('general.awaiting', 1) }}
                                                                {{ trans_choice('general.disbursement', 1) }}</span>
                                                        @endif
                                                        @if ($key->status == 'disbursed')
                                                            <span
                                                                class="label label-info">{{ trans_choice('general.active', 1) }}</span>
                                                        @endif
                                                        @if ($key->status == 'declined')
                                                            <span
                                                                class="label label-danger">{{ trans_choice('general.declined', 1) }}</span>
                                                        @endif
                                                        @if ($key->status == 'withdrawn')
                                                            <span
                                                                class="label label-danger">{{ trans_choice('general.withdrawn', 1) }}</span>
                                                        @endif
                                                        @if ($key->status == 'written_off')
                                                            <span
                                                                class="label label-danger">{{ trans_choice('general.written_off', 1) }}</span>
                                                        @endif
                                                        @if ($key->status == 'closed')
                                                            <span
                                                                class="label label-success">{{ trans_choice('general.closed', 1) }}</span>
                                                        @endif
                                                        ...
                                                        @if ($key->status == 'rescheduled')
                                                            <span
                                                                class="label label-info">{{ trans_choice('general.rescheduled', 1) }}</span>
                                                        @endif
                                                    @endif
                                                </td>
                        ...
                        <div class="panel-body table-responsive">
                            <table id="repayments-data-table" class="table  table-condensed table-hover">
                                <thead>
                                    <tr>
                                        <th>
                                            {{ trans_choice('general.collection', 1) }}
                                ...
                                <tbody>
                                    @foreach (\App\Models\LoanTransaction::where('borrower_id', $borrower->id)
                                                ...
                                                <ul class="icons-list">
                                                    <li class="dropdown">
                                                        ...
                                                        <ul class="dropdown-menu dropdown-menu-right">
                                                            <li>
                                                                <a
                                                                    href="{{ url('loan/transaction/' . $key->id . '/show') }}"><i
                                                                        class="fa fa-search"></i>
                                                            ...
                                                            <li>
                                                                <a href="{{ url('loan/repayment/' . $key->id . '/reverse') }}"
                                                                    class="delete"><i class="fa fa-minus-circle"></i>
                                                                    {{ trans('general.reverse') }}
                                                                </a>
                                                            </li>
                                    @endif
                                    </ul>
